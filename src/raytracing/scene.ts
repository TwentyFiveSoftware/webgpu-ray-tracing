	...
	public static generateRandomScene(): Scene {
		...
		spheres.push(
			new Sphere(
				new Vector3(-4, 1, 0),
				1,
				Material.solidDiffuse(new Vector3(0.6, 0.3, 0.1)),
			),
		);
		...
		spheres.push(
			new Sphere(
				new Vector3(4, 1, 0),
				1,
				Material.solidMetal(new Vector3(0.7, 0.6, 0.5)),
			),
		);
...
class Sphere {
	...
	public serializeToBytes(): ArrayBuffer {
		...
		new Uint8Array(buffer).set(
			new Uint8Array(this.center.serializeToBytes()),
			0,
		);
		bufferView.setFloat32(12, this.radius, true);
		new Uint8Array(buffer).set(
			new Uint8Array(this.material.serializeToBytes()),
			16,
		);
...
class Material {
	...
	public static solidDiffuse(color: Vector3): Material {
		return new Material(
			Material.MATERIAL_TYPE_DIFFUSE,
			Material.TEXTURE_TYPE_SOLID,
			0,
			color,
			new Vector3(0, 0, 0),
		);
	...
	public static checkeredDiffuse(color1: Vector3, color2: Vector3): Material {
		return new Material(
			Material.MATERIAL_TYPE_DIFFUSE,
			Material.TEXTURE_TYPE_CHECKERED,
			0,
			color1,
			color2,
		);
	...
	public static dielectric(refractionIndex: number): Material {
		return new Material(
			Material.MATERIAL_TYPE_DIELECTRIC,
			0,
			refractionIndex,
			new Vector3(0, 0, 0),
			new Vector3(0, 0, 0),
	...
	public serializeToBytes(): ArrayBuffer {
		...
		new Uint8Array(buffer).set(
			new Uint8Array(this.color1.serializeToBytes()),
			16,
		);
		new Uint8Array(buffer).set(
			new Uint8Array(this.color2.serializeToBytes()),
			32,
		);
...
export class RenderCallInfo {
	...
	constructor(
		...
		samplesPerComputePass: number,
		alreadyComputedSample: number = 0,
	) {
		this.width = width;
		this.height = height;
	...
	public serializeToBytes(): ArrayBuffer {
		...
		new Uint32Array(buffer).set(
			[
				this.width,
				this.height,
				this.maxRayTraceDepth,
				this.samplesPerComputePass,
				this.alreadyComputedSamples,
			],
			0,
		);
		return buffer;
