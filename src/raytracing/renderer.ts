...
export const startRayTracing = async (
	...
	const renderCallInfo = new RenderCallInfo(
		width,
		height,
		maxRayTraceDepth,
		samplesPerComputePass,
	);

	const renderCallInfoBuffer = engine.initializeBuffer(
		GPUBufferUsage.UNIFORM,
		renderCallInfo.serializeToBytes(),
	);
	const sceneBuffer = engine.initializeBuffer(
		GPUBufferUsage.STORAGE,
		Scene.generateRandomScene().serializeToBytes(),
	);

	const pixelBuffers = [
		engine.initializeBuffer(
			GPUBufferUsage.STORAGE,
	...
	const renderPipelineBindGroupLayout = engine.createBindGroupLayout(
		renderPipelineBindGroupEntries,
	);
	const renderPipeline = engine.createRenderPipeline(
		renderPipelineBindGroupLayout,
		vertexShaderCode,
		fragmentShaderCode,
	);
	const renderPassBindGroups = [1, 0].map((bufferIndex) =>
		engine.createBindGroup(renderPipelineBindGroupLayout, [
			{ binding: 0, resource: { buffer: renderCallInfoBuffer } },
			{ binding: 1, resource: { buffer: pixelBuffers[bufferIndex] } },
		]),
	);

	const computePipelineBindGroupLayout = engine.createBindGroupLayout(
		computePipelineBindGroupEntries,
	);
	const computePipeline = engine.createComputePipeline(
		computePipelineBindGroupLayout,
		computeShaderCode,
	);
	const computePassBindGroups = [
		[0, 1],
		[1, 0],
	].map(([sourceBufferIndex, targetBufferIndex]) =>
	...
	const requiredComputePassCount = Math.ceil(
		samplesPerPixel / samplesPerComputePass,
	);
	...
	for (let i = 1; i <= requiredComputePassCount; ++i) {
		...
		await engine.submit(
			engine.encodeComputePass(
				computePipeline,
				computePassBindGroups[i % 2],
				Math.ceil(width / COMPUTE_PASS_WORKGROUP_SIZE),
				Math.ceil(height / COMPUTE_PASS_WORKGROUP_SIZE),
			),
		);
		...
		logMessage(
			"[" +
				i.toString().padStart(samplesPerPixel.toString().length, " ") +
				`/${requiredComputePassCount} | ` +
		...
		await engine.submit(
			engine.encodeRenderPass(
				renderPipeline,
				renderPassBindGroups[i % 2],
				vertexBuffer,
			),
		);
	}
